...
?>
		...
		$cek_ac=mysql_query("SELECT a.nomor_perkara,
                                    ...
                                    h.pihak_id,
				    				i.telepon
                                    FROM sipp.perkara a, sipp.perkara_putusan b, sipp.hakim_pn c, sipp.panitera_pn d, sipp.perkara_panitera_pn e, sipp.perkara_hakim_pn f, sipp.perkara_akta_cerai g, sipp.perkara_pihak1 h, sipp.pihak i
                                    ...
                                    AND g.tgl_penyerahan_akta_cerai IS NULL
...
$tanggal_ac  = tgl_indo($r['tgl_akta_cerai']);
		...
		$cek=mysql_query("select *  from notif_perkara_pa.notif_ac_pihak_pertama where perkara_id='$r[perkara_id]' and nomor_perkara='$r[nomor_perkara]' and nomor_hp='$r[telepon]'");
		$jum=mysql_num_rows($cek);
		if($jum<1){
			
		$insert=mysql_query("insert into notif_perkara_pa.notif_ac_pihak_pertama (perkara_id, nomor_perkara,tanggal_ac,notif_pesan,nomor_hp, date_input) values ('$r[perkara_id]','$r[nomor_perkara]','$r[tgl_akta_cerai]','$pesan','$r[telepon]','".date('Y-m-d H:i:s')."')");

		$insert_ok=mysql_query("select * from notif_perkara_pa.notif_ac_pihak_pertama where perkara_id='$r[perkara_id]' and nomor_perkara = '$r[nomor_perkara]' and tanggal_ac = '$r[tgl_akta_cerai]' and notif_pesan ='$pesan' and nomor_hp='$r[telepon]'");
		$id=mysql_fetch_array($insert_ok);	
		$insert2=mysql_query("insert into notif_perkara_pa.notif_wa (no_tujuan, isi_pesan,status_kirim,detail_pengiriman,waktu_terkirim,id_notif) values ('$r[telepon]','$pesan','t',null,null,'$id[id_notif_ac_pihak_pertama]')");	
		//exit;	
		}
